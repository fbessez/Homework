...
struct
  ...
  *)
  ...
  (*  assertExn (test_name, test_fn, exp) = t, where t is a test 
  *   named test_name that succeeds if test_fn() raises exp.
  *)
  fun assertExn (test_name : string, 
                 test : unit -> 'a, 
                 exp : exn) : test =
  let
  ...
  (*  assertNothing (test_name, test_fn) = t, where t is a test that always 
  *   succeeds.  The only thing that can go wrong is that test_fn() raises an
  *   exception.
  *)
  fun assertNothing (
      test_name : string, 
